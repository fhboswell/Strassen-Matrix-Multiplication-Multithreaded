...
//tests the acuracy of the implementations against the naive algorithm implementation
void proof ();
...
void strassen_recursion_multi_plus(int *MATA, int *MATB, int*MAT, int N, int cutoff){
    ...
    if(N > 1){
        ...
        //malloc is allocate memory calloc allocates and sets all to 0
        //this is very useful so that you dont have to set them to 0 later with a loop
        ...
        //the smaller matricies need to be passed to the original algorithem in the same form as the others, therefore they
        //must be reshaped like this for convience
        reshape(A11, A11A, N);
        reshape(B11, B11B, N);
        reshape(B22, B22B, N);
        reshape(A22, A22A, N);
        ...
        //an important note
        //strassen's algorithm is very ineffecent if it is terminated in single unit multiplication so instead the cutoff is around 128 for the cases that i tested
        //there is a formula that can calculate the optimum cutoff
        
        if(N  <= cutoff){
            //printMAT(P1, N);
            
            //makle threads
            //this is the "plus" optimization
            //this is the addition to the books algirthim that I through up and decided to implement
            //it worked too, this created a large speed up
            ...
            //i was thinking of farming this out to threads too but i didnt want to create thrashing
        ...
        //getting ready for the 7 multiplications
...
void test(){
    ...
    int smaller_test_sizes[] ={ 32, 64, 128, 256, 512, 1024, 2048, 4092};
